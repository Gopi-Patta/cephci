...
* `--store` - stores the cluster configuration as snapshot under cephci/rerun directory
   ```
      ...
      --store
   ```
* `--reuse <rerun\snapshot_name>` - set reuse cli arg to use the deployed cluster from cephci/rerun/ directory
   ```
      python run.py --osp-cred <cred_file> \
      --instances-name <instances_name> \
      --global-conf <conf_file> \
